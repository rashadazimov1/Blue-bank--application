...
public class RabbitMqConfig {
    ...
    @Value("accountFirstStepQueue")
    String queueName;

    @Value("accountRoutingKey")
    String routingKey;
    ...
    Queue secondStepQueue() {
        return new Queue("accountSecondStepQueue", true);
    }

    @Bean
    Queue thirdStepQueue() {
        return new Queue("accountThirdStepQueue", true);
    }
    ...
    Binding secondBinding(Queue secondStepQueue, DirectExchange exchange){
        return BindingBuilder.bind(secondStepQueue).to(exchange).with("accountSecondRouting");
    }

    @Bean
    Binding thirdBinding(Queue thirdStepQueue, DirectExchange exchange){
        return BindingBuilder.bind(thirdStepQueue).to(exchange).with("accountThirdRouting");
    }
